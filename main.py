...
# ==========================================
# 4. UI: HEADER CON DISCLAIMER LEGAL
# ==========================================
...
st.markdown(f"""
    ...
    <div class="fixed-header">
        <div class="top-strip">
            ...
            <span class="warning-text">⚠️ PRECIOS ESTIMADOS (IA) | REVISIÓN FINAL</span>
        </div>
...
with st.sidebar:
    ...
    if not st.session_state.cart:
        ...
        st.divider()
        st.caption("⚠️ Los precios son calculados por IA y pueden tener variaciones al confirmar con Martín.")
...
5. **DISCLAIMER (IMPORTANTE):**
   - Siempre actúa asumiendo que el precio es una COTIZACIÓN ESTIMADA.
   - Si preguntan exactitud: "Martín te confirma el centavo final, yo te doy el aproximado hoy".
...
with c2:
    if MIC_AVAILABLE:
        ...
        st.warning("⚠️ Sin micrófono (Falta librería)")
        audio_text = None
        ...
        with st.spinner("Cotizando..."):
            try:
                ...
                if items_nuevos:
                    ...
                    st.dataframe(pd.DataFrame(items_nuevos)[['cantidad','producto','precio_unit']], hide_index=True)
                    st.markdown("---")
