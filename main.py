...
# --- 5. CARGA Y LIMPIEZA DE DATOS (OPTIMIZADO) ---
...
def load_data():
    try:
        ...
        # Limpieza profunda para reducir tokens y acelerar a Gemini
        df = df.dropna(how='all', axis=1) # Borra columnas vacías
        df = df.dropna(how='all', axis=0) # Borra filas vacías
        df = df.fillna("")
        return df 
    except Exception:
        return None
...
if raw_data is not None and not raw_data.empty:
    try:
        # Convertimos a string CSV compacto para que la IA lea más rápido
...
# --- 6. SISTEMA DE MÉTRICAS (THREADING ROBUSTO) ---
...
def enviar_a_google_form_background(cliente, monto, oportunidad):
    """Ejecuta el envío en un hilo separado con control de errores estricto"""
    if URL_FORM_GOOGLE and "docs.google.com" in URL_FORM_GOOGLE:
        try:
            ...
            # Timeout corto para no dejar procesos colgados
            requests.post(URL_FORM_GOOGLE, data=payload, timeout=3)
        except Exception:
            pass # Falla silenciosa para no afectar al usuario
...
🚨 **REGLA IMPOSITIVA:** Precios CSV son NETOS. Obligatorio sumar "+ IVA" en la salida.

🧠 **PROTOCOLO DE VENTA CRUZADA (CROSS-SELLING INTELIGENTE):**
1.  **ANÁLISIS DE COMPLEMENTOS:**
    * Si pide Chapas -> Ofrece Tornillos y Aislantes.
    * Si pide Perfiles -> Ofrece Discos de corte y Electrodos.
2.  **GESTIÓN DE STOCK (REGLA DE ORO):**
    * **SI ESTÁ EN LISTA:** Coticéalo activamente con precio. Ej: "Tengo los tornillos autoperforantes a $X. ¿Los sumo?".
    * **SI NO ESTÁ EN LISTA:** NO inventes precio. Dile: "Agrego los complementarios a tu pedido para que Martín los cotice". Y ponlos en el resumen de WhatsApp.

🏆 **ESCALA DE DESCUENTOS:**
* **$200k - $299k:** "Estás cerca del MAYORISTA (15% OFF). ¿Qué más necesitas?".
...
if "chat_session" not in st.session_state:
    try:
        # MODELO GEMINI 2.5 PRO (MANTENIDO SEGÚN SOLICITUD)
        ...
        try:
            ...
            st.error("Error crítico de conexión con IA.")

# --- 9. INTERFAZ DE USUARIO (CON FEEDBACK MEJORADO) ---
...
if prompt := st.chat_input("Ej: 20 chapas para San Jorge..."):
    ...
    try:
        ...
        with st.chat_message("assistant", avatar="👷‍♂️"):
            ...
            # Este spinner se mantiene visible SOLO mientras se procesa el primer byte.
            with st.spinner("Miguel está analizando stock y precios..."):
                try:
            ...
            # Iteramos el stream. El spinner desaparece automáticamente al entrar aquí.
